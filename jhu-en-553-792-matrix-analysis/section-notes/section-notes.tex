...
For the first exercise, you will need this theorem.
...
\end{thm}
We covered this theorem in class, however, we only stated and proved the theorem when $\H$ was {\bf finite-dimensional}. Otherwise, it is exactly the same. The intuition should be clear in finite dimensions.
\begin{exc}
...
\begin{exc}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ||y||^D \geq || \mid y \mid ||^D
        \end{align*}
        \item Finally, show that $||y||^D \leq || \mid y \mid ||^D$, completing the proof.
    \end{enumerate}
\end{exc}

\begin{exc}
    Using the previous exercise, show that if $||\cdot||$ is absolute, then it is monotone. Let's break this up as well.
    \begin{enumerate}
        \item Show that $||x|| = \max_{||u||^D = 1} |u|^T |x|$. {\it Hint: use the fact that $||x|| = {||x||^D}^D$ and the Hahn-Banach theorem to ensure existence of a maximizer $\hat{u}$.}
        \item Using the fact that $|x| \leq |y|$ element-wise, and that
        \begin{align*}
            ||x|| = \frac{|\hat{u}|^T |x|}{||\hat{u}||},
        \end{align*}
        conclude that $||x|| \leq ||y||$.
    \end{enumerate}
\end{exc}
